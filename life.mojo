        ...
        if started or step:
            ...
            for row in range(grid.rows):
                for col in range(grid.cols):
                    ...
                    cell_fill_color = pygame.Color(
                        Python.int(255 * Int(grid[row, col, 0])),
                        Python.int(255 * Int(grid[row, col, 1])),
                        Python.int(255 * Int(grid[row, col, 2])),
                        Python.int(255),
                    )
                    pygame.draw.rect(
                        window,
...
def main():
    print(
        "Tap <ENTER> to start a new game, <SPACE> to pause/resume, or <ESC> to"
        " quit."
    )
